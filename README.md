...
The Experimentation framework is split into three stages (and three easy commands):
...
1.  **Broad Model and Configuration Testing**
    -   Command to run:
        ...
        ```
    -   Many different experiments (different models/configurations) are tested on a few specific backtests.
    -   Results will be recorded to `hummingbot_files/results/backtest_results.csv`. **This file is cumulative and will grow over time.**
    ...
    -   *Note: Every experiment has a unique hash. If two experiments are the same, they will also have the same hash. The script will skip experiments with an existing hash in `backtest_results.csv` to save time.*
...
2.  **Deep Analysis**
    -   Command to run:
        ...
        ```
    -   The top 5 models (by PnL, by default) will be tested over many different date ranges and timeframe configurations.
    -   Results will be recorded to `hummingbot_files/results/top_model_results.csv`. **Warning: This file is not cumulative and will be overwritten each time.**

3.  **Analyze Experiment Results**
    -   Once experiments are finished, start the custom analysis dashboard with this command:
        ```bash
